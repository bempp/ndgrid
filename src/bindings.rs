...
mod grid {
    ...
    use ndelement::{ciarlet::CiarletElement, types::{Continuity, ReferenceCellType}, ciarlet::LagrangeElementFamily, traits::{ElementFamily, FiniteElement}};
    use std::ffi::c_void;
    ...
    unsafe fn single_element_grid_new_from_raw_data<T: RealScalar>(
        coordinates: *const T, npts: usize, gdim: usize,
        cells: *const usize,
        ncells: usize,
        cell_type: u8,
        ...
        let points_per_cell = LagrangeElementFamily::<T>::new(geometry_degree, Continuity::Standard).element(ReferenceCellType::from(cell_type).unwrap()).dim();
        ...
        let grid = Box::into_raw(Box::new(
            SingleElementGrid::<T, CiarletElement<T>>::new_from_raw_data(
                &coordinates, gdim, &cells, ReferenceCellType::from(cell_type).unwrap(), geometry_degree
            )
        )) as *const c_void;
    ...
    pub unsafe extern "C" fn single_element_grid_new_from_raw_data_f32(
        ...
        single_element_grid_new_from_raw_data::<f32>(coordinates, npts, gdim, cells, ncells, cell_type,
                                  geometry_degree, DType::F32)
    }
    ...
    pub unsafe extern "C" fn single_element_grid_new_from_raw_data_f64(
        ...
        single_element_grid_new_from_raw_data::<f64>(coordinates, npts, gdim, cells, ncells, cell_type,
                                  geometry_degree, DType::F64)
    }
